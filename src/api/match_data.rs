...
impl<'a> MatchData<'a> 
{
    pub fn prepare(&self) -> Result<Vec<MatchDataRequest>, Box<dyn std::error::Error>> 
    {
        ...
        let match_data_requests: Vec<MatchDataRequest> = self.matches.iter()
                                                                     .map(|x| MatchDataRequest { url: format!("https://americas.api.riotgames.com/lol/match/v5/matches/{m}",m = x) })
                                                                     .collect();
        println!(
